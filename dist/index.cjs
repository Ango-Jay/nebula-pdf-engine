...
async function renderToSvg(element, options) {
	...
	return await (0, satori.default)(element, {
		width: options.width,
...
function renderToPng(svgString) {
	return new _resvg_resvg_js.Resvg(svgString, {
...
async function assemblePdf(pages, options = {}) {
	...
	const pdfDocument = await pdf_lib.PDFDocument.create();
...
var AssetResolver = class {
	...
	async resolveImage(src) {
		...
		if (src.startsWith("http")) {
			...
			const absolutePath = node_path.default.isAbsolute(src) ? src : node_path.default.join(process.cwd(), src);
			buffer = await node_fs_promises.default.readFile(absolutePath);
		}
		const base64 = `data:image/png;base64,${(await (0, sharp.default)(buffer).png().toBuffer()).toString("base64")}`;
...
function Image({ src, width, height, style, alt }) {
	return /* @__PURE__ */ (0, preact_jsx_runtime.jsx)("img", {
		src,
		width,
		height,
		style: {
