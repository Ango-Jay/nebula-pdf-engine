...
async function main() {
  ...
  const tableNode = h(Table, { columns, data, options: { stripe: true, headerRepeat: true } });
  
  // Create a minimal tree containing just the table
  const element = h(Page, { padding: 40 }, tableNode);
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    console.log(`Page ${i + 1} has ${pageNodes.length} nodes`);
    for (const node of pageNodes) {
      if ((node as any).props && (node as any).props._segment) {
        const seg = (node as any).props._segment;
        console.log(`  Table segment with ${seg.rows.length} rows`);
        if (seg.rows.length > 0) {
          console.log(`    First row: ${seg.rows[0].id}`);
          console.log(`    Last row: ${seg.rows[seg.rows.length - 1].id}`);
        }
      } else {
        console.log(`  Node type: ${(node as any).type}`);
      }
    }
  }
}
